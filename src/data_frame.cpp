...
void DataFrame::sortBy<int>(const string header, const char op)
{
    ...
    if (op == '<'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
            return stoi(lrow[headerValue]) < stoi(rrow[headerValue]);});
    }
    else if (op == '>'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
...
void DataFrame::sortBy<float>(const string header, const char op)
{
    ...
    if (op == '<'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
            return stof(lrow[headerValue]) < stof(rrow[headerValue]);});
    }
    else if (op == '>'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
...
void DataFrame::sortBy<double>(const string header, const char op)
{
    ...
    if (op == '<'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
            return stod(lrow[headerValue]) < stod(rrow[headerValue]);});
    }
    else if (op == '>'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
...
void DataFrame::sortBy<long>(const string header, const char op)
{
    ...
    if (op == '<'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
            return stol(lrow[headerValue]) < stol(rrow[headerValue]);});
    }
    else if (op == '>'){
        sort(this->rowVector.begin(), this->rowVector.end(), \
        [=](Row& lrow, Row& rrow){
...
int DataFrame::getMax<int>(const string header)
{
    ...
    for (auto row: this->rowVector){
        max = stoi(row[headerValue]) >= max ?
        stoi(row[headerValue]) : max;
    }
...
float DataFrame::getMax<float>(const string header)
{
    ...
    for (auto row: this->rowVector){
        max = stof(row[headerValue]) >= max ?
        stof(row[headerValue]) : max;
    }
...
double DataFrame::getMax<double>(const string header)
{
    ...
    for (auto row: this->rowVector){
        max = stod(row[headerValue]) >= max ?
        stod(row[headerValue]) : max;
    }
...
long DataFrame::getMax<long>(const string header)
{
    ...
    for (auto row: this->rowVector){
        max = stol(row[headerValue]) >= max ?
        stol(row[headerValue]) : max;
    }
...
int DataFrame::getMin<int>(const string header)
{
    ...
    for (auto row: this->rowVector){
        min = stoi(row[headerValue]) <= min ?
        stoi(row[headerValue]) : min;
    }
...
float DataFrame::getMin<float>(const string header)
{
    ...
    for (auto row: this->rowVector){
        min = stof(row[headerValue]) <= min ?
        stof(row[headerValue]) : min;
    }
...
double DataFrame::getMin<double>(const string header)
{
    ...
    for (auto row: this->rowVector){
        min = stod(row[headerValue]) <= min ?
        stod(row[headerValue]) : min;
    }
...
long DataFrame::getMin<long>(const string header)
{
    ...
    for (auto row: this->rowVector){
        min = stol(row[headerValue]) <= min ?
        stol(row[headerValue]) : min;
    }
